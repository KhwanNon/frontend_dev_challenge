...
- Lifecycle ของ GetxController ผูกกับ route ส่วน Lifecycle ของ State ผูกกับ widget ดังนั้น State ของ widget สามารถสร้างหรือลบทิ้งได้ตลอดในขณะที่ controller ยังอยู่เพราะมันผูกกับ route
- ตัวอย่างบั๊กคือ RES-103 ใน DealDetailsController.onInit มีการเรียก ever(cartService.itemCount, ...) แต่ไม่ได้เก็บ Worker ไว้ และไม่ได้ dispose ใน onClose จึงทำให้การสมัคร listen ถูก stack ขึ้นเรื่อยๆ
...

**Q2. When does wrapping a large subtree in a single `Obx` hurt you? How do you decide how tightly to scope reactivity?**
- Obx จะ rebuild ทุกอย่างที่มันครอบอยู่ ดังนั้นยิ่ง subtree ใหญ่ ก็ทำให้ใช้ทรัพยากรเครื่องมากขึ้น และอาจจะแสดงอาการจอกระตุกได้
- เราควรเอา Obx ครอบแค่ Widget ที่เราต้องการให้มีการเปลี่ยนแปลงเท่านั้น เช่น ครอบแค่ Text()
...
...
- จากที่เข้าใจคือ แบบเดิมเราแสดงเวลาตาม UTC เราควรแปลงเป็นเวลาขอใประเทศไทยก่อน 
- จะแก้ isToday ให้รับค่าวันเข้าไป ไม่ให้มันดึง DateTime.now() เอง เพราะเวลาเทสเราจะได้สามารถกำหนดเวลาได้ ไม่งั้นเราต้องแก้เทสตาม DateTime.now()
...
| Item | Time |
| --- | --- |
| ทำความเข้าใจโจทย์และวางแผน | 2 ชม. |
| ทำความเข้าใจโปรเจคและเครื่องมือที่ใช้คร่าวๆ | 2 ชม. |
| RES-101 | 1 ชม. |
| RES-102 | 15 นาที |
| RES-103 | 50 นาที |
| RES-107 | 30 นาที |
